...
## All predictors
```{r}
predictors <- site.sum %>% select(cover_LC,cover_TA,canopy_MA,canopy_TA,rugosity,scar.BM,scar.DEN) # should it matter if I use site level summary data vs. full follow dataset (where site-level data has been joined?)
...
stat_summary(fun.y = mean, geom="point", shape=5, size=3) + labs(y="Feeding Rate (bites/hr)",x="",title="Feeding Rate") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_signif(comparisons = list(c("Antigua", "Barbuda"),c("Antigua","Bonaire"),c("Barbuda","Bonaire")), map_signif_level=TRUE) + facet_grid(.~Species)
ggsave("bp_fr.png")
```
...
```{r}
# calculate predicted feeding rates from Bruggemann/Bozec:
FR.vir15=1089-17*15-56
FR.vir20=1089-17*20-56
FR.vir25=1089-17*25-56
...
ANUint.vir <- data %>% filter(Island=="Antigua"&Phase=="i"&Species.Code=="stop")
ANUint.vet <- data %>% filter(Island=="Antigua"&Phase=="i"&Species.Code=="qup")
FR.anu.vir <- lm(FR~Length.cm, data=ANUint.vir)
FR.anu.vet <- lm(FR~Length.cm, data=ANUint.vet)
summary(FR.anu.vet)
#FR.vet = 1047-12*L
summary(FR.anu.vir)
#FR.vir = 445-8*L

FR.vet15ANU=1047-12*15
FR.vet20ANU=1047-12*20
FR.vet25ANU=1047-12*25

FR.vir15ANU=445-8*15
FR.vir20ANU=445-8*20
FR.vir25ANU=445-8*25

# calculate predicted bite sizes from Bruggemann/Bozec

BS.vir15=5.257*10^-4*15^2
BS.vir20=5.257*10^-4*20^2
BS.vir25=5.257*10^-4*25^2

BS.vet15=4.013*10^-4*15^2
BS.vet20=4.013*10^-4*20^2
BS.vet25=4.013*10^-4*25^2
...
areagrazed.site <- feedingrates %>% group_by(Site) %>% dplyr::summarize(observed=sum(areagrazed.ANU)/1000,predicted=sum(areagrazed.pub)/1000)
...
ggplot(data=AG, aes(x=Site, y=area.grazed, fill=feeding.rates)) +
    geom_bar(stat="identity", position=position_dodge()) + theme_bw() + scale_fill_manual(values=c("black","lightslategrey")) + labs(y="Area Grazed (m2)")
```
