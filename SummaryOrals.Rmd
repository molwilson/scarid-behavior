...
ANUint.vir <- data2 %>% filter(Island=="Antigua"&Phase=="i"&Species.Code=="stop")
ANUint.vet <- data2 %>% filter(Island=="Antigua"&Phase=="i"&Species.Code=="qup")
...
feedingrates.vir.ANU <- data2 %>% filter(Species.Code=="stop"&Phase=="i") %>% mutate(FR.ANU = 445-8*Length.cm) %>% mutate(FR.pub=1089-17*Length.cm-56) %>% mutate(BS=5.257*10^-4*Length.cm^2)
feedingrates.vet.ANU <- data2 %>% filter(Species.Code=="qup"&Phase=="i") %>% mutate(FR.ANU = 1047-12*Length.cm) %>% mutate(FR.pub=3329-33*Length.cm) %>% mutate(BS=4.013*10^-4*Length.cm^2)
...
areagrazed.site <- feedingrates %>% group_by(Site) %>% summarise(areagrazed.site.ANU=sum(areagrazed.ANU))
```
