...
aovFRvir <- aov(FR ~ Island, data=vir, white.adjust=TRUE) 
...
```{r, message=FALSE,warning=FALSE,echo=FALSE}
ggplot(vetvir20, aes(x=Island, y=BR)) + geom_boxplot() + facet_grid(.~Species) +
stat_summary(fun.y = mean, geom="point", shape=5, size=3) + labs(y="Bite Rate (bites/sec)",x="",title="Bite Rate while Grazing by Island and Species") + theme_bw() + theme(plot.title = element_text(hjust = 0.5))
```

```{r, message=FALSE,warning=FALSE,echo=FALSE}
vet<- vetvir %>% filter(Species.Code=="qup")
aovBRvet <- aov(BR ~ Island, data=vet, white.adjust=TRUE) 
...
vir<- vetvir %>% filter(Species.Code=="stop")
aovBRvir <- aov(BR ~ Island, data=vir, white.adjust=TRUE) 
...
```{r, message=FALSE,warning=FALSE,echo=FALSE}
ggplot(vetvir20, aes(x=Island, y=g.frac)) + geom_boxplot() + facet_grid(.~Species) +
stat_summary(fun.y = mean, geom="point", shape=5, size=3) + labs(y="Time fraction",x="",title="Fraction of Time Spent Grazing by Island and Species") + theme_bw() + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r, message=FALSE,warning=FALSE,echo=FALSE}
vet<- vetvir %>% filter(Species.Code=="qup")
aovGIvet <- aov(for.bites ~ Island, data=vet, white.adjust=TRUE) 
...
vir<- vetvir %>% filter(Species.Code=="stop")
aovGIvir <- aov(for.bites ~ Island, data=vir, white.adjust=TRUE) 
...
```{r, message=FALSE,warning=FALSE,echo=FALSE}
ggplot(vetvir20NS,aes(x=scar.BM,y=BR))+facet_grid(.~Species)+geom_point(shape=18)+geom_smooth(method=loess, fullrange=FALSE)+ggtitle("Bite rate while grazing by scarid biomass") + theme_minimal() +labs(y="Bite rate (bites/sec)",x="Scarid Biomass (g/100m2)") + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r, message=FALSE,warning=FALSE,echo=FALSE}
ggplot(vetvir20NS,aes(x=scar.BM,y=for.bites))+facet_grid(.~Species)+geom_point(shape=18)+geom_smooth(method=loess, fullrange=FALSE)+ggtitle("Grazing intensity by scarid biomass") + theme_minimal() +labs(y="Bites/foray",x="Scarid Biomass (g/100m2)") + theme(plot.title = element_text(hjust = 0.5))
```
