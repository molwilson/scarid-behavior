...
sum_id <- read.csv(here("data","sum_id.csv"), stringsAsFactors=F) %>%
  select(-X) %>%
  filter(length_cm>=15)
sum_site_sp_ph <- read.csv(here("data","sum_site_sp_ph.csv"), stringsAsFactors=F) %>%
  select(-X) 
```
...
pca_pred <- prcomp(na.omit(sum_site %>% 
              ...
              select(ma_ind,ta_cover,ta_canopy,lc_cover,rugosity,scar_den,scar_bm,)), 
  center = T,scale = T)
...
pca_bent <- prcomp(na.omit(sum_site %>% 
              remove_rownames %>% 
              column_to_rownames(var="site") %>%
              select(ma_ind,ta_cover,ta_canopy,lc_cover,rugosity,)), 
  center = T,scale = T)
...
sum_ssp_pca1 <- sum_site_sp_ph %>% left_join(select(sum_site_pca1,site,pc1,pc2), by = "site")
write.csv(sum_id_pca1,file=here("data/sum_id_pca1.csv"))
write.csv(sum_ssp_pca1,file=here("data/sum_ssp_pca1.csv"))
```
...
```{r}
ggplot(filter(sum_id_pca1, species_code != "rbp" & island != "Barbuda" & length_cm>=16 & length_cm<=24), aes(x = ma_ind,y=fr)) +
  facet_grid(.~species) +
  geom_smooth(method="loess") +
  geom_point(shape=18) +
...
```{r}
lme_fr <- lme(fr~phase+length_cm+species+scar_bm+pc1+pc2,
...
gamm<-gamm(fr~phase+s(length_cm)+s(pc1)+s(pc2)+s(scar_bm)+s(scar_den), 
           ...
           data=filter(sum_id_pca1, species_code=="stop"),
           random=list(site=~1),
           method="REML"
           )
summary(gamm$gam)
...
```
```{r,eval=F}
gamm<-gamm(fr_mean~phase+species+s(pc1)+s(pc2)+s(scar_bm)+s(scar_den), 
           family=gaussian(link=identity),
