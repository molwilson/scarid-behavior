...
pca_pred <- prcomp(na.omit(sum_site %>% 
              ...
              select(ma_ind,ta_cover,ta_canopy,lc_cover,rugosity,scar_den,scar_bm,spear_daily)),
  center = T,scale = T)
...
```{r,eval=F}
gamm<-gamm(fr~phase+s(length_cm)+s(pc1_bent)+s(pc2_bent)+s(scar_bm)+s(scar_den), 
           family=gaussian(link=identity),
           data=filter(sum_id_pca, species_code=="qup"),
           random=list(site=~1),
           method="REML"
           )
summary(gamm$gam)
...
```
```{r,eval=F}
gamm1<-gamm(fr_mean~phase+species+s(pc1_bent)+s(pc2_bent)+s(scar_bm)+s(scar_den), 
           family=gaussian(link=identity),
           data=filter(sum_ssp_pca, species_code!="rbp" & island!="Barbuda"),
           random=list(site=~1),
           method="REML"
           )
summary(gamm1$gam)
