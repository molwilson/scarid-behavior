...
* As reefs become increasingly degraded/threatened, there is growing interest in understanding how much herbivory is needed to sustain coral reef health (e.g Karr et al., McClanahan et al.)
...
* Herbivore behavior has been shown to be impacted by…
    * Predator presence (Catano, Madin)
    * Presence of other herbivores (Gil)
    * Resource availability (Carlson, Tootell)
* But there is vary limited data on how herbivore feeding behaviors might vary across reef systems, as well as the environmental factors associated with grazing differences
* Here we document variability in multiple components of scarid feeding behavior across reef sites in three Caribbean islands and explore/enhance our understanding of potential underlying drivers
...
grid.arrange(left = "Feeding rate (bites / minute)",
  ...
  arrangeGrob(
    ...
    arrangeGrob(s4, bottom = "EAM height"),
    arrangeGrob(s5, bottom = "EAM % cover"),
    arrangeGrob(s7, bottom = "MA % cover"),
...
* Caveats + limitations: all these different factors are understandablycorrelated in observational work
