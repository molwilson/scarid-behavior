...
exp_sp <- fish %>% expand(nesting(species,family),site) # all possible species/site combinations
...
exp_meter <- ab_benthic %>% 
  ...
  summarize()
exp_cat <- ab_benthic %>% expand(nesting(code),site)
...
bent_site <- bent_site %>% left_join(rug_site,by="site") %>% bind_rows(bon_benthic)
```
