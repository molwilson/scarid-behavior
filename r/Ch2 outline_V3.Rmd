...
**Abstract:** Herbivorous fish increase coral growth and survival by grazing down algal competitors. As coral reef health continues to decline globally, maximizing herbivory on reefs has become a primary management goal. However, existing management targets assume consistent grazing behavior across different reef systems. Here I document the grazing behavior of two scarid species at 13 sites in Antigua, Barbuda, and Bonaire. My analyses show significant differences in overall feeding rates, time spent grazing, and intensity of grazing across reef sites, which may alter the ecological impact of a given scarid population. I propose several potential mechanisms for these grazing differences, including variations in bite content which may make bite rates a misleading measure of overall grazing impact. As we set management targets to enhance herbivory on coral reefs, it is critical that we are accurately estimating the grazing impact of scarid populations. This paper demonstrates the variability of grazing behavior across different reef sites and calls for further investigation of the drivers and ecological implications of these inconsistencies.
...
Feeding rates, time spent grazing, and grazing concentration varied significantly across sites within both *S. vetula* and *S. viride* populations (Fig. 2). [Fig. 2A also shows substantially lower feeding rates in most Antiguan (*S. vetula* and *S. viride*) and Barbudan (*S. vetula*) sites as compared standard values used in Caribbean herbivory models - *if I include reference lines...*]. Differences in site-level feeding rates stemmed from differences in the fraction of time that fish speng grazing (Fig. 2B) as opposed to differences in bite rates while activitely feeding, which remained relatively constant accross sites (appendix). Interspecific differences in feeding rates and grazing intensity reflect the different feeding morphologies/functional roles of *S. vetula* and *S. viride*.

These behavioral variations may have important ecological consequences and management implications. If we use grazing data from one area to predict the impact of a given species in other parts of the region, we may be overlooking important behavioral differences and misrepresenting grazing levels of different scarid populations. Feeding rates are used in herbivory models to calculate the area grazed by a given fish community. In the Caribbean case, most standard feeding rate equations are obtained from data collected in Bonaire. My results suggest that applying Bonaire feeding rates to other Caribbean reef systems may overestimate grazing impact.

Grazing concentration also plays a key role in determining the ecological impact of coral reef herbivores. More concentrated grazing may be more effective in removing algae and clearing substrate for growing or newly recruiting corals than bites dispersed throughout the reefscape. [**add citations here**]
...
gamm1 <- gamm(fr_mean ~ species_code + phase + s(length_m, k=5) + s(pc1_all, k=5) + s(pc2_all, k=5), 
...
gamm2 <- gamm(fr_mean ~ species_code + phase + s(length_m, k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm3 <- gamm(fr_mean ~ species_code + phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5) + s(pc2_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm4 <- gamm(fr_mean ~ species_code + phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5) + s(pc2_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm5 <- gamm(fr_mean ~ species + phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm6 <- gamm(fr_mean ~ phase + s(length_m, k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm7 <- gamm(fr_mean ~ phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm8 <- gamm(fr_mean ~ species + phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm9 <- gamm(fr_mean ~ species + phase + s(length_m, k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm10 <- gamm(fr_mean ~ species + phase + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm11 <- gamm(fr_mean ~ phase + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
With initial phase individuals only:
```{r, echo=F, message=F, warning=F}
m_data <- sum_ssp_pca %>% filter(species_code!="rbp")

gamm1 <- gamm(fr_mean ~ species_code + s(length_m, k=5) + s(pc1_all, k=5) + s(pc2_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm2 <- gamm(fr_mean ~ species_code + s(length_m, k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm3 <- gamm(fr_mean ~ species_code + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5) + s(pc2_all, by = factor(species_code), k=5),
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm4 <- gamm(fr_mean ~ species_code + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5) + s(pc2_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm5 <- gamm(fr_mean ~ species + s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm6 <- gamm(fr_mean ~ s(length_m, k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm7 <- gamm(fr_mean ~ s(length_m, by = factor(species_code), k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm8 <- gamm(fr_mean ~ species + s(length_m, by = factor(species_code), k=5) + s(pc1_all, k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm9 <- gamm(fr_mean ~ species + s(length_m, k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm10 <- gamm(fr_mean ~ species + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gamm11 <- gamm(fr_mean ~ s(length_m, by = factor(species_code), k=5) + s(pc1_all, by = factor(species_code), k=5), 
           family = gaussian(link=identity),
           random = list(island=~1),
           method = "REML",
           data = m_data
...
gtab_fr <- as.data.frame(rbind(g1,g2,g4,g5,g6,g7,g8,g9,g10,g11)) %>% setNames(c("g","Model", "AICc", "Adj. R2")) %>% arrange(AICc)
```
