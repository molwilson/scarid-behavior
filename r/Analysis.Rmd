...
sum_id_sub <- sum_id %>%
  filter(length_cm >= 15 & length_cm <= 30 & species_code %in% c("qup", "stop") & phase == "i")

ggplot(data = sum_id_sub, aes(x = site, y = length_cm, group = site)) +
  geom_boxplot() +
  facet_grid(. ~ species)

ggplot(data = sum_id_sub, aes(x = length_cm, group = site, fill = site)) +
  geom_density(alpha = 0.1)

lm <- lm(data = sum_ssp_pca, pc1_all ~ length_mean)
summary(lm) # not significantly correlated with PC1
```
