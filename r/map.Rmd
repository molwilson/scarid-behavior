...
# google API (not sure if I can register this permanently?)
...
Alternate (geocoding) option
```{r}
#world <- map_data("world") # df
ll.islands <- geocode(c("Antigua","Barbuda","Bonaire"))
islands.x <- ll.islands$lon
islands.y <- ll.islands$lat

ggplot() +
  theme_bw() +
  geom_sf(data = world, fill = "gray80", col = "gray80") +
  annotation_scale(location = "tr", width_hint = 0.25) +
  coord_sf(xlim = c(-90, -58), ylim = c(10, 33)) +
  geom_point(aes(x = islands.x, y = islands.y), color = "#f24db3", size = 5, shape = 1) +
  labs(x = "", y = "",title = "")

```

### Island-level details
