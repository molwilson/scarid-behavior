...
```{r}
ggplot(data,aes(x=scar.BM,y=FR))+facet_grid(.~Species)+geom_smooth(method="lm")+geom_point(shape=18)+ggtitle("Feeding rate by scarid biomass") + theme_minimal() +labs(y="Feeding rate (bites/hr)",x="Scarid Biomass (g/100m2)") + theme(plot.title = element_text(hjust = 0.5))
```
